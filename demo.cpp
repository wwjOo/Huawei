...
int Astartest(char arr[N][N], int startX, int startY, int &endX, int &endY) 
{
    Timer astarT;
    int res = 0; //-1:快到帧时间了 0:失败 >0:路径长度
    int dx[4] = {1, -1, 0, 0};
    int dy[4] = {0, 0, 1, -1};
...
void Strategy_Robot_Berth_Choose(int zhen, Robot &r)
{
    ...
    else
    {
        ...
        {
            int num = berth[r.target_berth].neighbors.size() > 2 ? 2 : berth[r.target_berth].neighbors.size();
            for(int k=0; k<num; k++) //选择邻近的两个泊位
            {
                if(berth[berth[r.target_berth].neighbors[k].second].robot_num >= 2) continue; //每个泊位限制最多3个机器人进行服务
...
int Strategy_Boat_BtoB_Choose(int zhen, Boat &b) //-1:没有合适泊位，0～9泊位索引id
{
    ...
    if( (berth[b.pos].robot_num == 0 && berth[b.pos].stackedcargos.size() == 0) || (berth[b.pos].stackedcargos.size() == 0 && boat_capacity - b.num > 20))
    {
        ...
        for(int i=0; i<berth_num; i++)
        {
            ...
            if( berth[i].curboat == -1 && berth[i].transport_time + 500 < 14999 - id)
            {
    ...
    for(int r=0; r<robot_num; r++)
    {
        ...
        if(robot[i].status == 1 && robot[i].goods == 0 && robot[i].tx == 0)
        {
            ...
            if(target.value != -1)
            {
                ...
                if(res == 1 && (robot[i].dir.size() + id + 3 >= target.eid || robot[i].dir.size() >= berth[robot[i].target_berth].buf[target.x][target.y] + 60)) res = -1;
        ...
        else if(robot[i].status == 1 && robot[i].goods == 0 && robot[i].tx != 0 && !berth[robot[i].target_berth].belongcargos.empty())
        {
            Cargo c = berth[robot[i].target_berth].belongcargos.top();

            std::deque<std::vector<int>> pbuf;
            int len = Astartest(ch, robot[i].x,robot[i].y, c.x, c.y);

            if(0 < len && len < paths[i].size() && c.priority > robot[i].curcargo.priority)
            {
                cerr <<"success - " << endl;
                int res = Astar(ch, zhen, robot[i].x,robot[i].y, c.x, c.y, i, 0, paths[i], t);
                if(res == 1)
                {
                    //把最新的拿出来
                    berth[robot[i].target_berth].belongcargos.pop();
                    //把原目标放回去
                    berth[robot[i].target_berth].belongcargos.push(robot[i].curcargo);
                    //更改目标    
                    robot[i].curcargo = c;
...
int PullCargo(int zhen, Timer &t)
{
    ...
    for(int k=0; k<robot_num; k++)
    {
        ...
        if (robot[i].status == 1 && robot[i].goods == 1 && robot[i].tx == 0)
        {
            ...
            if(res == 1 && (robot[i].dir.size() >= berth[best_berth].buf[robot[i].x][robot[i].y] + 25)) res = -1;
